...
- (void)setupShareViewWithItems:(NSArray *)items {
    ...
    if (self.itemsPerLine < items.count) {
        //分割线
        self.lineView.frame = CGRectMake(0, shareScrollView.frame.size.height + shareScrollView.frame.origin.y, self.frame.size.width, 1);
        
        shareScrollView = [[XMNShareScrollView alloc] initWithFrame:CGRectMake(0, self.lineView.frame.origin.y+self.lineView.frame.size.height+self.distance, self.frame.size.width, [XMNShareScrollView heightForScrollView])];
        shareScrollView.shareDelegate = self;
...
- (void)_updateshareContentViewFrame:(BOOL)hidden {
    ...
    if (self.distanceHeight != 0.0) {
        self.distanceHeight = height;
    }
    self.itemsContentView.frame = CGRectMake(0, self.distanceHeight, CGRectGetWidth(self.bounds), CGRectGetHeight(self.itemsContentView.bounds));
